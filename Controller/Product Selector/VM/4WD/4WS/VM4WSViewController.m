...
#import "VM6000ViewController.h"
#import "VM8000ViewController.h"

@interface VM4WSViewController ()

@end

@implementation VM4WSViewController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if( [key isEqualToString:@"6000"] ) {
        [self performSegueWithIdentifier: @"vm6000" sender:self];
    } else if( [key isEqualToString:@"8000"] ) {
        [self performSegueWithIdentifier: @"vm8000" sender:self];
    } else {
        NSArray *arrPrd = [prd allKeys];
        NSString *p = [arrPrd objectAtIndex:0];
        self.selProduct = prd[p];
        //[self performSegueWithIdentifier: @"4wssingle" sender:self];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if( [[segue identifier] isEqualToString:@"4wssingle" ] ) {
    ...
    } else if( [[segue identifier] isEqualToString:@"vm6000" ] ) {
        VM6000ViewController *ctrl = segue.destinationViewController;
        ctrl.title = @"Vertical Mast";
        ctrl.products = self.selProduct;
        [self addBackButton:ctrl];
    } else if( [[segue identifier] isEqualToString:@"vm8000" ] ) {
        VM8000ViewController *ctrl = segue.destinationViewController;
        ctrl.title = @"Vertical Mast";
        ctrl.products = self.selProduct;
        [self addBackButton:ctrl];        
    }
